...
function Popup() {
    ...
    const handleLoaderData = function () {
        HandleInputValue();
        if (getValue) {
    ...
    return (
        <>
            ...
            <div className='modal' onClose={() => setOpen(false)} style={{ display: open !== false ? 'flex' : 'none' }}>
                ...
                <input type="text" className={inputColor ? "modal__danger" : "modal__input"} onChange={(e) => setValue(e.target.value)} placeholder={t("enteryourname")} />
                <label className='modal__label' htmlFor="name">{t("yourphone")} <span>*</span> </label>
                <input type="tel" className={inputColor ? "modal__danger" : "modal__input"} onChange={(e) => setValue(e.target.value)} placeholder='+998 99 888-77-66' />
                <div className='modal__btns'>
                    <button className='btn reject' onClick={() => setOpen(false)}>{t("reject")}</button>
                    <button className='btn dispatch' disable={disabledBtn} onClick={handleLoaderData}>{send}</button>
                </div>
            </div>
        </>
    )
