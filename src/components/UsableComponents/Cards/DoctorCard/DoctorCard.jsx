...
function DoctorCard({data}) {
    ...
    return (<NavLink to={`/doctors/${data.doctor_id}`} key={data.doctor_id} className='doctor__card'>
        ...
        <div data-aos="zoom-in" data-aos-duration="4000" className='fullname'><span>{lang === 'uz' ? name_uz : name_uz}</span>
            <br/>{lang === 'uz' ? surname_uz : surname_uz} <br/> {lang === 'uz' ? lastname_uz : lastname_uz}</div>
    </NavLink>)
}
